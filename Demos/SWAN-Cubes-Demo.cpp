...
struct CameraController : SWAN::InputFrame
{
	...
	if(key.Code == SWAN::G) SWAN::Display::Resize(640, 480);
	else if(key.Code == SWAN::H) SWAN::Display::Resize(1280, 720);
	else if(key.Code == SWAN::Y) SWAN::Display::Resize(320, 240);
    }
	...
	if(key.Code == SWAN::W) cam.moveForw(speed);
	if(key.Code == SWAN::S) cam.moveForw(-speed);
	if(key.Code == SWAN::A) cam.moveRight(-speed);
	if(key.Code == SWAN::D) cam.moveRight(speed);
	if(key.Code == SWAN::Spacebar) cam.moveUp(speed);
	if(key.Code == SWAN::LShift) cam.moveUp(-speed);
    }
...
struct Grid {
	...
	shader.setUniform({"transform", SWAN::Transform()});
    }
