...
struct XVertex {
	...
	operator SWAN::Vertex() {
		return SWAN::Vertex(pos, (hasUV ? UV : vec2()),
		                    (hasNorm ? norm : vec3()));
	}
};
...
unique_ptr<Mesh> Import::OBJ(std::string filename, Import::Settings s) {
	...
	if(s.smoothNormals) {
		...
		for(unsigned i = 0; i < rVerts.size(); i++) {
			...
			auto v = SWAN::Util::CollectIterIf(
			    rVerts.begin(),
			    rVerts.end(), [& target = rVerts[i]](Vertex v)->bool {
	...
	auto genAABB = [](vector<Vertex> v) -> AABB {
		...
		res.min -= vec3(0.002, 0.002, 0.002);
		res.max += vec3(0.002, 0.002, 0.002);
	...
	if(s.createAABB || s.colType == ColWrapper::COL_AABB) {
	    ...
	    // Really ugly (and probably slow) method, but gives good
  results.
	    // To find the proper center of the mesh, we create its AABB
  and use
	    // its center as the sphere's.
	    auto box = genAABB(rVerts);
	    sphere.center = (box.max - box.min) / 2.0f;

	    sphere.radius = std::max(box.max.x - box.min.x,
  std::max(box.max.y - box.min.y, box.max.z - box.min.z)) / 2.0f;
