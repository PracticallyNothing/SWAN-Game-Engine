...
struct ShaderUniform {
	...
	ShaderUniform(std::string name, vec2 v2) : name(name) {
		data.v2 = v2;
		type    = T_VEC2;
	}
	ShaderUniform(std::string name, vec3 v3) : name(name) {
		data.v3 = v3;
		type    = T_VEC3;
	}
	ShaderUniform(std::string name, vec4 v4) : name(name) {
		data.v4 = v4;
		type    = T_VEC4;
	}

	ShaderUniform(std::string name, mat2 m2) : name(name) {
		data.m2 = m2;
		type    = T_MAT2;
	}
	ShaderUniform(std::string name, mat3 m3) : name(name) {
		data.m3 = m3;
		type    = T_MAT3;
	}
	ShaderUniform(std::string name, mat4 m4) : name(name) {
		data.m4 = m4;
		type    = T_MAT4;
	}
...
class Shader {
  public:
	...
	void setUniformData(const std::string& name, const mat2& data, GLboolean transposed = GL_FALSE);
	void setUniformData(const std::string& name, const mat3& data, GLboolean transposed = GL_FALSE);
	void setUniformData(const std::string& name, const mat4& data, GLboolean transposed = GL_FALSE);

	void setUniformData(const std::string& name, mat2&& data, GLboolean transposed = GL_FALSE);
	void setUniformData(const std::string& name, mat3&& data, GLboolean transposed = GL_FALSE);
	void setUniformData(const std::string& name, mat4&& data, GLboolean transposed = GL_FALSE);
