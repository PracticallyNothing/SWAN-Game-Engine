...
/*
        ...
        Collision CheckCollision(BoundingSphere a, BoundingSphere b){
                ...
                res.happened = length(a.center - b.center) < a.radius +
   b.radius;
                res.distance = length(a.center - b.center) - (a.radius +
   b.radius);
...
Collision CheckCollision(AABB a, LineSegment b) {
	...
	res.happened = Util::RangesOverlap(a.min.x, a.max.x, b.start.x, b.end.x) &&
...
/*
        Collision CheckCollision(BoundingSphere s, AABB b){
                ...
                vec3 p(max(b.min.x, min(s.center.x, b.max.x)),
