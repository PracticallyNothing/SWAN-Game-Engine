...
namespace SWAN
{
	...
	Intersection FindIntersection(Segment segment, Plane plane)
	{
		...
		double t = (Dot(plane.normal, plane.getPointOnPlane()) - Dot(plane.normal, segment.start))
	...
	Intersection FindIntersection(AABB aabb, Segment segment)
	{
		...
		res.happened = Util::RangesOverlap(aabb.min.x, aabb.max.x, segment.start.x, segment.end.x)
	...
	{
		HSResult rMin = HalfSpaceTest(aabb.min, plane);
		HSResult rMax = HalfSpaceTest(aabb.max, plane);

		double distance = Dot(aabb.min, plane.normal) - plane.offset;
		...
		if(rMin == HSResult::On) {
			...
			res.happened = true;
			res.type = Intersection::Type::Point;
			res.point = aabb.min + plane.normal * distance;
		}
	...
	{
		auto h1 = HalfSpaceTest(t2.points[0], t1),
		     h2 = HalfSpaceTest(t2.points[1], t1),
		     h3 = HalfSpaceTest(t2.points[2], t1);
		if(h1 == h2 && h2 == h3 && h1 != HSResult::On) {
			return Intersection();
		}

		h1 = HalfSpaceTest(t1.points[0], t2);
		h2 = HalfSpaceTest(t1.points[1], t2);
		h3 = HalfSpaceTest(t1.points[2], t2);
		if(h1 == h2 && h2 == h3 && h1 != HSResult::On) {
			return Intersection();
		}
