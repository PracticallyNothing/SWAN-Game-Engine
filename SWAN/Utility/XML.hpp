...
namespace Util {
	...
	class XMLTag {
	  public:
		...
		~XMLTag() {
			// DEBUG_PRINT(std::string("        [XMLTag::~XMLTag()] Deleting ")
			// +
			// getFullName());
		...
		std::string getAttrib(const std::string& name) const {
			return attribs.find(name)->second;
		}
	  ...
	  private:
		...
		void findTagsWithName(const std::string& name,
		                      std::vector<const XMLTag*>& v) const;
	};
	...
	struct XML {
		...
		std::vector<const XMLTag*> findTagsWithName(const std::string& name) {
			return root.findTagsWithName(name);
		}
	};
