...
namespace SWAN {
	...
	struct CxArray {
	    ...
	    constexpr T& at(unsigned index) { static_assert(index > 0 && index < N, "SWAN::Util::CxArray::at() called with out of range index."); return arr[index]; }
	    ...
	    constexpr const T& at(unsigned index) const { static_assert(index > 0 && index < N, "Const SWAN::Util::CxArray::at() called with out of range index."); return arr[index]; }
