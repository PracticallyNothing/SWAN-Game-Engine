	...
	namespace AL
	{
		...
		struct Source {
		  ...
		  private:
			...
			bool GetBoolValue(ALenum en) const
			{
				ALint i;
				alGetSourcei(id, en, &i);
				return i == AL_TRUE ? true : false;
			}
			/// Retrieves the integer value of the given enum.
			int GetIntValue(ALenum en) const
			{
				ALint i;
				alGetSourcei(id, en, &i);
				return i;
			...
			float GetFloatValue(ALenum en) const
			{
				ALfloat f;
				alGetSourcef(id, en, &f);
				return f;
			}
			/// Retrieves the 3-dimensional value of the given enum.
			fvec3 GetVec3Value(ALenum en) const
			{
				fvec3 v;
				alGetSource3f(id, en, &v.x, &v.y, &v.z);
				return v;
			}
