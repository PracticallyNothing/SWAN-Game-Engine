...
namespace SWAN {
	...
	void PreviousLine() {SeekBOL(); Left(1); SeekBOL();}

	/// Moves the cursor to the beginning of the next line.
	void NextLine() {SeekEOL(); Right(1);}

	/// Moves the cursor down a given amount of lines.
	void Down(unsigned amt) { for(int i = 0; i < amt; i++) Down(); }
	/// Moves the cursor up a given amount of lines.
	void Up(unsigned amt) { for(int i = 0; i < amt; i++) Up(); }
	...
	void Reset() { Size = 0; CursorPosition = 0; }
	...
	unsigned GetCurrentLineLength() const { int e = FindEOL(); int b = FindBOL(); return e-b;}

	/// Retrieves the current line as a string.
	String GetCurrentLine() const { int b = FindBOL(); return String(buff + b, GetCurrentLineLength()); }
