...
namespace SWAN {
	...
	void Init(int width, int height, const std::string& title) {
	    ...
	    // Create the window
	    detail::window =
		SDL_CreateWindow(title.c_str(), /* Window title. 
						 * NOTE: It must be a C-style string (char*) so it must be 
						 * converted with a call to "c_str()". */
	...
	void SetClearColor(float red, float green, float blue, float alpha) {
	    Log("Display",
		std::string("Setting background color to {r: ") + std::to_string(red)
