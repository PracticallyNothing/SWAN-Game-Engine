...
namespace SWAN {
	struct Ray { glm::vec3 start, direction; };
	struct Plane { glm::vec3 normal; double offset; };
	...
	inline Collision CheckCollision (Ray ray, AABB box)         { return CheckCollision (box, ray);   }
	inline Collision CheckCollision (Plane plane, AABB box)     { return CheckCollision (box, plane); }
