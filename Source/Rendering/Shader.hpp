...
namespace SWAN {
	...
	struct ShaderUniform {
		...
		enum Type {
			T_INT, T_BOOL, T_FLOAT, T_DOUBLE,

			T_VEC2, T_VEC3, T_VEC4,
			T_MAT2, T_MAT3, T_MAT4,

			T_TRANSFORM,

			T_POINTLIGHT, T_DIRLIGHT, T_SPOTLIGHT,
		} type;
		...
		ShaderUniform(ShaderUniform&& other)
			: name(std::move(other.name)), data(std::move(other.data)), type(std::move(other.type)) {}
