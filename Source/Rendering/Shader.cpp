...
void Shader::setUniformData(const std::string& name, glm::mat2& data,
                            GLboolean transposed) {
	if(hasUniform(name))
		glUniformMatrix2fv(uniforms[name], 1, transposed, &data[0][0]);
}

void Shader::setUniformData(const std::string& name, glm::mat3& data,
                            GLboolean transposed) {
	if(hasUniform(name))
		glUniformMatrix3fv(uniforms[name], 1, transposed, &data[0][0]);
}

void Shader::setUniformData(const std::string& name, glm::mat4& data,
                            GLboolean transposed) {
	if(hasUniform(name))
