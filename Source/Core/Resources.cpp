...
bool LoadFromFile(const std::string& filename) {
    ...
    for (auto tag : meshes) {
        ...
        if (fileIt == tag->attribs.end()) {
            std::cout << "ERROR: <Mesh> tag has no file attribute, it will be "
                         "skipped.\n";
            continue;
        }
        ...
        if (nameIt == tag->attribs.end() || !nameIt->second.size()) {
            std::cout << "Warning: <Mesh> tag with empty or nonexistant name "
                         "attribute.\n"
        ...
        detail::meshes.emplace((name.length() ? name : file),
                               Import::OBJ(file, Import::Settings{true, true}));
    }
    ...
    for (auto tag : textures) {
        ...
        if (fileIt == tag->attribs.end()) {
            std::cout << "ERROR: <Texture> tag has no file attribute, it will "
                         "be skipped.\n";
            continue;
        }
        ...
        auto nameIt = tag->attribs.find("name");
        if (nameIt == tag->attribs.end() || !nameIt->second.size()) {
            std::cout << "Warning: <Texture> tag with empty or nonexistant "
                         "name attribute.\n"
                      << "         The filename (\"" << dir + fileIt->second
                      << "\") will be used as the name.\n";
        }
    ...
    for (auto tag : bitmapFonts) {
        ...
        if (fileIt == tag->attribs.end()) {
            std::cout << "ERROR: <BitmapFont> tag has no file attribute, it "
                         "will be skipped.\n";
            continue;
        }
        ...
        auto nameIt = tag->attribs.find("name");
        if (nameIt == tag->attribs.end() || !nameIt->second.size()) {
            std::cout << "Warning: <BitmapFont> tag with empty or nonexistant "
                         "name attribute.\n"
    ...
    for (auto tag : shaderTags) {
        ...
        {  // ----------- <Attribute/> -----------
            ...
            for (auto tag : tags) res->addAttrib(tag->getAttrib("name"));
        }
        res->linkShaders();
        ...
        res->use();
        {
            {  // ----------- <Uniform/> -----------
                ...
                for (auto tag : tags) res->addUniform(tag->getAttrib("name"));
            }
            {  // -------- <ArrayUniform/> --------
