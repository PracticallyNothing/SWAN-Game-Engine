...
class EventManager {
  ...
  public:
	...
	inline void updateByType(const std::string& type) {
		updateIf([type & ](const EventListener& l)
		             ->bool{ return l.type == type });
	}
	...
	inline auto getActiveListeners() {
		return getListenersByPredicate([](const EventListener& l) -> bool {
			return l.active() && !l.expired();
		});
	}

	/// Returns all EventListeners that have expired
	inline auto getExpiredListeners() {
		return getListenersByPredicate(
		    [](const EventListener& l) -> bool { return l.expired(); });
	}

	/// Returns all EventListeners that have a matching type
	inline auto getListenersByType(const std::string& type) {
		return getListenersByPredicate([type & ](const EventListener& l)->bool {
			return l.type == type;
		});
	}
