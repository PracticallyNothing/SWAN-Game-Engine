...
CreateImage (
  ...
  const Texture* texture) {
	...
	EventListener onMouseEnter (
	  [el = res]()->bool {
		  return GUIElement::focused == nullptr && GetCurrMouseState ().mousesOver (el->x, el->y, el->w, el->h) && !GetPrevMouseState ().mousesOver (el->x, el->y, el->w, el->h);
	  },
	  [el = res] {
	...
	EventListener onMouseLeave (
	  [el = res]()->bool {
		  return GUIElement::focused == el && !GetCurrMouseState ().mousesOver (el->x, el->y, el->w, el->h) && GetPrevMouseState ().mousesOver (el->x, el->y, el->w, el->h);
	  },
	  [el = res] {
...
CreateButton (
  ...
  EventListener::ActionFuncT onRelease) {
	...
	EventListener onMousePress (
	  [el = res]()->bool {
		  return GUIElement::focused == el && GetCurrMouseState ().LMB && !GetPrevMouseState ().LMB;
	  },
	  [ el = res, active ] {
