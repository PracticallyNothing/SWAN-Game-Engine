...
int main(int argc, char** argv)
{
	...
	for(int i = 1; i < argc; i++) {
		...
		for(int i = 0; i < img.width * img.height; i += 4)
			file << (int) img.data[i * 4 + 0] << "," << (int) img.data[i * 4 + 1] << ","
			     << (int) img.data[i * 4 + 2] << "," << (int) img.data[i * 4 + 3] << ","
			     << (int) img.data[i * 4 + 4] << "," << (int) img.data[i * 4 + 5] << ","
			     << (int) img.data[i * 4 + 6] << "," << (int) img.data[i * 4 + 7] << ","
			     << (int) img.data[i * 4 + 8] << "," << (int) img.data[i * 4 + 9] << ","
			     << (int) img.data[i * 4 + 10] << "," << (int) img.data[i * 4 + 11] << ","
			     << (int) img.data[i * 4 + 12] << "," << (int) img.data[i * 4 + 13] << ","
			     << (int) img.data[i * 4 + 14] << "," << (int) img.data[i * 4 + 15]
			     << ",\n";
